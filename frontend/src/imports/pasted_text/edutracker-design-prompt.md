EduTracker — Complete AI UI/UX Design Prompt
Project Overview

Design a modern educational platform called EduTracker.

The design should look almost identical to premium online learning platforms such as Coursera, Udemy, Skillshare and Google Career Certificates while following the layout shown in the reference images.

Use a clean white interface with soft shadows, rounded corners, spacious layouts, modern typography, subtle animations and professional dashboards.

The design must be fully responsive for

Desktop
Tablet
Mobile

Everything should be built as reusable components.

Design Style

Minimal

Modern

Professional

Educational

Clean

Google Material inspired

Rounded cards

Soft shadows

Large spacing

Beautiful gradients

Smooth transitions

Micro interactions

Interactive hover effects

Premium quality

Color Palette

Primary

Blue #2563EB

Secondary

Black #111827

Background

#F8FAFC

Card

White

Border

#E5E7EB

Success

#22C55E

Warning

#F59E0B

Danger

#EF4444

Purple Accent

#8B5CF6

Typography

Headings

Poppins Bold

Body

Inter

Small Text

Inter Regular

Buttons

Semi Bold

Border Radius

16px

Cards

20px

Buttons

12px

Shadows

Soft modern shadows

No harsh borders

Floating dashboard cards

Sidebar Navigation

Exactly like the reference.

Logo

EduTracker

Menu

Dashboard

My Courses

Explore Courses

Certificates

Assignments

Quizzes

Calendar

Messages

Bookmarks

Downloads

Achievements

Settings

Help Center

Invite Friends Card

Logout

Active menu uses

Black background

White icon

Rounded pill

Top Navigation

Search Bar

Notification

Messages

Dark Mode Toggle

Profile Avatar

User Menu

Breadcrumb Navigation

Complete Pages
1. Landing Page

Large Hero Section

Navigation

Features

Statistics

Popular Courses

Testimonials

Partners

Pricing

FAQ

Footer

Buttons

Enroll Now

Get Started

Browse Courses

Hero Illustration

(Image Placeholder)

2. Sign In

Exactly like screenshot

Google Login

Email

Password

Forgot Password

Continue Button

Remember Me

Illustration

(Image Placeholder)

3. Sign Up

Same style

Name

Email

Password

Confirm Password

Google Signup

Illustration Placeholder

Terms

Already have account

4. Forgot Password

Email

OTP Verification

Reset Password

Success Screen

5. Student Dashboard

Exactly like screenshot

Sections

Welcome Card

Learning Progress

Weekly Activity

Performance Graph

Upcoming Classes

Recent Courses

Continue Learning

Announcements

Quick Actions

Course Statistics

Calendar Widget

Leaderboard

Daily Goals

Right Profile Panel

Profile Picture

Progress Ring

Daily Activity

Study Hours

6. Explore Courses

Exactly like screenshot

Search

Filters

Categories

Course Grid

Course Card

Course Image Placeholder

Instructor

Rating

Students

Lessons

Duration

Price

Enroll Button

Favorite Button

Pagination

7. Course Details

Exactly like screenshot

Large Banner Image Placeholder

Video Preview Placeholder

Course Description

Modules

Lessons

Instructor

Reviews

Requirements

Learning Outcomes

Enroll Button

Share Button

Bookmark

Sidebar

Course Info

Duration

Modules

Projects

Certificate

Language

Level

8. Learning Page

Exactly like screenshot

Video Player Placeholder

Notes Panel

Tabs

Overview

Notes

Resources

Q&A

Transcript

Downloads

Code Editor Placeholder

Progress Tracker

Course Timeline

Right Sidebar

Learning Progress

Completed Lessons

Current Lesson

Upcoming Lessons

Quiz Status

Assignments

Discussion

9. My Courses

Exactly like screenshot

Current Courses

Completed

Wishlist

Archived

Progress Bars

Resume Button

Certificate Button

10. Certificates

Exactly like screenshot

Certificate Cards

Download

Share

Verify

Preview

Search

Categories

Certificate Image Placeholder

11. Assignments

Upcoming

Submitted

Pending

Late

Upload Assignment

Drag Drop

Status

Marks

Feedback

12. Quiz Page

Instructions

Question Navigation

Timer

MCQ

Coding Question

Progress

Submit

Result

Solutions

Leaderboard

13. Calendar

Study Schedule

Upcoming Classes

Deadlines

Meetings

Assignments

Google Calendar Sync UI

14. Notifications

Announcements

Assignment Reminder

Quiz Reminder

Course Update

Certificates

15. Messages

Inbox

Mentor Chat

Discussion

Attachments

Emoji

Voice Message UI

16. User Profile

Profile Picture

Bio

Skills

Achievements

Certificates

Enrolled Courses

Settings

Edit Profile

17. Settings

General

Account

Password

Notifications

Privacy

Appearance

Language

Billing

Delete Account

18. Admin Dashboard

Overview

Students

Instructors

Courses

Revenue

Analytics

Charts

Reports

Users

Course Management

Certificate Approval

19. Instructor Dashboard

Create Course

Upload Videos

Assignments

Student List

Analytics

Revenue

Reviews

Messages

20. Help Center

FAQ

Support Tickets

Documentation

Contact

Live Chat

Components

Reusable Components

Navbar

Sidebar

Buttons

Cards

Course Card

Progress Card

Certificate Card

Chart Card

Statistics Card

Avatar

Breadcrumb

Tabs

Accordion

Modal

Toast

Dropdown

Pagination

Timeline

Progress Ring

Charts

Calendar

Video Player

Comment Box

Rating

Search

Filters

Badges

Tags

Alerts

Skeleton Loader

Empty State

Dashboard Widgets

Course Progress

Weekly Learning

Study Hours

Completion Rate

Recent Activity

Upcoming Quiz

Assignments

Achievements

Certificates

Leaderboard

Quick Actions

Charts

Line Chart

Area Chart

Bar Chart

Pie Chart

Donut Chart

Radar Chart

Heatmap

Calendar Heatmap

Progress Ring

Learning Timeline

Empty States

Every page must include

Illustration Placeholder

Friendly Message

Action Button

Image Placeholders

Do NOT use real images.

Instead create placeholders for

Hero Banner

Course Thumbnail

Instructor Avatar

Student Avatar

Certificate Preview

Course Banner

Video Thumbnail

Profile Image

Illustration

Dashboard Graphics

I will replace every placeholder later in Figma.

Responsive Layout

Desktop

1440px

Laptop

1280px

Tablet

768px

Mobile

390px

Auto Layout everywhere.

Use proper constraints.

Figma Structure

Pages

01 Cover

02 Design System

03 Components

04 Landing

05 Authentication

06 Student Dashboard

07 Courses

08 Learning

09 Certificates

10 Assignments

11 Quiz

12 Calendar

13 Notifications

14 Profile

15 Settings

16 Admin Dashboard

17 Instructor Dashboard

18 Prototype

19 Assets

20 Icons

Design System

Create reusable components with variants.

Buttons

Primary

Secondary

Outline

Ghost

Danger

Success

Inputs

Default

Focused

Error

Disabled

Cards

Course Card

Dashboard Card

Analytics Card

Certificate Card

Badges

Completed

Pending

Locked

Premium

Progress

0%

25%

50%

75%

100%

Icons

Outlined

Filled

Prototype

Create clickable navigation between all pages.

Include hover states, active states, loading states, success states, error states, empty states, and responsive variants.

The final result should look like a premium production-ready LMS with the same visual quality, spacing, card layouts, sidebar navigation, dashboard organization, course pages, learning experience, and certificate screens shown in the provided reference images, while using reusable Auto Layout components and image placeholders that can easily be replaced later in Figma.